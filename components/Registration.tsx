...
interface Props {
  ...
  regType: boolean; // true for registration, false for profile change
}
const fetcher = (url) => fetch(url).then((r) => r.json());

const Registration = ({ initialPatient, regType }: Props): ReactElement => {
  ...
  return (
    <div className="container">
      ...
      {!regType && successSave && (
        <div
          className="flex justify-center my-6 bg-green-100 rounded-md border border-green-500 text-green-700 px-4 py-3"
          role="alert"
        >
          <p className="font-bold">Saved Changes Successfully</p>
        </div>
      )}
        ...
        <input
          defaultValue={
            regType
              ? ''
              : formatInTimeZone(
                  fromUnixTime(initialPatient.birthday),
                  'UTC',
                  'yyyy-MM-dd'
                )
          }
